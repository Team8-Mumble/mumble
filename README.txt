...
Mumble uses 15 - 144 kbits/s incoming and outgoing when transmitting speech.
The default setting in a bandwidth requires 55 kbit/s per stream.
This is partly dependent on the compression scheme chosen: when the default 40 Kbps is used, sent data remains about 40 Kbps while received data is around half that for one stream.
If there are 10 users in a channel, your incoming bandwidth requirement will be ( 10 * 144 kbit/s ).
For the outgoing bandwidth, the maximum requirement in your server will be ( 144 kbit/s * 9 users * 10 streams = 12960 kbit/s )
